...
| | |
|---|---|
| Performance | **94** (median of 5; runs vary 87–96) |
| Accessibility | **100** |
| Best practices | **100** |
| SEO | **100** |
...
### Student work — the section the design rests on

`WORK` in `lib/content.ts` is **empty on purpose**, and the gallery hides itself
until it is filled. The direction this page follows concluded that the product
is sold by showing what people create, and that the student outputs should *be*
the visual identity. Nothing in that gallery can be stock, and nothing can be
generated by anyone who is not a student: on a page whose entire claim is "look
what students made", an image no student made is the one lie it cannot survive.

Assets go in `public/work/`. 16:9 for campaign stills and ad-film frames, 4:5 or
9:16 for social and UGC. Video works too: give it `type: 'video'`, an mp4 under
3MB and a poster frame.
